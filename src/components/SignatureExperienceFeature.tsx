...
export function SignatureExperienceFeature({ embedded = false }: { embedded?: boolean }) {
  ...
  const comingSoon = signatureValenciaExperience.comingSoon;
    ...
    <div className="card-signature grid gap-0 overflow-hidden lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-center p-8 sm:p-10 lg:p-12">
        ...
        <h2 className="font-display text-3xl font-semibold text-gray-900 sm:text-4xl mt-1">
          {signatureValenciaExperience.title}
        </h2>
        <p className="mt-4 text-base leading-relaxed text-gray-600 italic">
          &ldquo;{signatureValenciaExperience.tagline}&rdquo;
        </p>
        <p className="mt-4 text-sm leading-relaxed text-gray-700">
          {comingSoon
            ? "Our flagship Signature Experience is in development — a curated small-group Valencia day that will become the primary recommendation on this site. The homepage is designed so this slot becomes your first choice without a redesign."
            : "The experience we would honestly recommend to a first-time cruise passenger who wants the very best of Valencia in a single port day."}
        </p>
        ...
        {comingSoon ? (
          <div className="placeholder-widget mt-6 text-left">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Historic centre highlights, Turia Gardens, City of Arts photography and a local lunch
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            {signatureValenciaExperience.benefits.slice(0, 4).map((b) => (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href={SIGNATURE_EXPERIENCE_PATH} className="btn-accent">
            {comingSoon ? "Learn more →" : `Discover ${signatureValenciaExperience.title} →`}
          </Link>
          {comingSoon && (
