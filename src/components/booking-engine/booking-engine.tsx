...
export function BookingEngine({ shipsByDate }: BookingEngineProps) {
  ...
  const handleDateConfirmed = (isoDate: string) => {
    ...
    setLiveMessage(
      returning && nextShip
        ? `${label} selected. ${nextShip.name} still visits Valencia. Returning to booking.`
        : nextShip
          ? `${label} selected. ${nextShip.name} is visiting Valencia. Continuing to confirm your ship.`
