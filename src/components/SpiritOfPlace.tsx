    ...
    <section className="section-padding bg-gradient-to-b from-coastal-50 via-white to-white border-b border-coastal-100">
      <div className="container-wide">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            <p className="section-eyebrow">Mediterranean Valencia</p>
            <h2 className="section-title mt-2 max-w-xl">{spiritOfPlace.title}</h2>
            {spiritOfPlace.body.map((paragraph) => (
              <p key={paragraph.slice(0, 24)} className="mt-4 text-base leading-relaxed text-gray-600 first:mt-6 first:text-gray-700">
                {paragraph}
              </p>
            ))}
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {[
                { emoji: "🍚", label: "Paella birthplace" },
                { emoji: "🌳", label: "Turia Gardens" },
                { emoji: "🎨", label: "City of Arts contrast" },
              ].map((item) => (
